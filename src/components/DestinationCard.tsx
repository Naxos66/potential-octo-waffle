  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-gold/20 bg-slate-900/70 transition duration-300 hover:-translate-y-1 hover:border-gold/60 hover:shadow-gold">
      ...
      <div className="space-y-3 p-5">
        <p className="text-xs uppercase tracking-[0.15em] text-gold-soft">{destination.epoch}</p>
        <h3 className="text-xl font-semibold text-white">{destination.title}</h3>
        <p className="text-sm text-slate-300">{destination.description}</p>
        <div className="flex flex-wrap gap-2">
          {destination.tags.map((tag) => (
            <span key={tag} className="rounded-full border border-gold/30 px-3 py-1 text-xs text-gold-soft">
              {tag}
            </span>
          ))}
        </div>
